...
function LegacyInspector({ busy, job, onJobMutate, onManageAssets }) {
  return <aside className="inspector detail-rail"><div className="inspector-head"><span className="eyebrow">Trabajo anterior · Sin clasificar</span><h2>{job.title}</h2><p>{job.job_id}</p></div><div className="dialog-note legacy-note"><WorkflowIcon name="archive" size={18}/><span>Este trabajo conserva el flujo anterior. Puedes revisar o aprobarlo; no se reescribirá como plan nuevo.</span></div><JobReviewPanel busy={busy} job={job} onManageAssets={onManageAssets} onMutate={onJobMutate}/></aside>;
}

export default function ProductionWorkspace({ busy, health, items, jobs, onAction, onChecklist, onEdit, onJobMutate, onManageAssets, templates }) {
  ...
  const active = records.find((record) => record.id === selectedId) || visible[0] || records[0];
  return (
    <>
      ...
      <div className="content-grid production-workspace"><section className="queue-panel"><header className="section-head"><div><span className="eyebrow">Cola canónica</span><h2>Producción social</h2></div><p><b>{items.filter((item) => item.stage !== 'draft').length}</b> planes · <b>{jobs.length}</b> trabajos</p></header>{visible.length ? <div className="queue-stage-groups">{groups.map((group) => <section className="queue-stage-group" key={group.id}><header><h3>{group.label}</h3><span>{group.records.length}</span></header><div className="content-card-list">{group.records.map((record) => <ProductionCard active={active?.id === record.id} item={record.item} job={record.job} key={record.id} onSelect={() => setSelectedId(record.id)} template={record.template}/>)}</div></section>)}</div> : <div className="empty-queue compact-empty"><WorkflowIcon name="queue" size={32}/><h3>Sin piezas en esta etapa</h3><p>Cambia el filtro o crea un plan desde la matriz.</p></div>}</section>{active?.item ? <ContentInspector busy={busy} item={active.item} job={active.job} onAction={onAction} onChecklist={onChecklist} onEdit={onEdit} onJobMutate={onJobMutate} onManageAssets={onManageAssets} template={active.template}/> : active?.job ? <LegacyInspector busy={busy} job={active.job} onJobMutate={onJobMutate} onManageAssets={onManageAssets}/> : <ContentInspector/>}</div>
    </>
  );
}
